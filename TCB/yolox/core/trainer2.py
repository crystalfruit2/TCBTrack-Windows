...
class Trainer2:
    ...
    def before_train(self):
        ...
        self.train_loader, settings = self.exp.get_data_loader2(
        ...
        model = self.exp.get_model2(settings=self.settings)
        #A = torch.randn([2,3,800,1440])
        #B = torch.randn([2,1000,5])
        #out = model(A,B)
        #logger.info(
        #    "Model Summary: {}".format(get_model_info(model, self.exp.test_size))
        #)
        model.to(self.device)
        #model.head.togpu(self.device)
        ...
        if self.start_epoch!=0:
            try:        # mot dataset with ids
                ...
                self.train_loader = self.exp.get_data_loader2(
                batch_size=self.args.batch_size,
                is_distributed=self.is_distributed,
                no_aug=True
            )
    ...
    def after_iter(self):
        if (self.iter + 1) % self.exp.print_interval == 0:
            ...
            if (self.iter + 1) % 100 == 0 and self.rank==0:
                for k, v in loss_meter.items():
                    if(v.latest==0):
                        break
                    self.tblogger.add_scalar("{}".format(k), v.latest, self.iter + 1+self.max_iter*(self.epoch))
            """
        ...
        if (self.iter + 1) % 2000 == 0:
            self.save_ckpt(ckpt_name="latest2")
    ...
    def evaluate_and_save_model(self):
        ...
        self.model.train()
        self.model = self.exp.freeze(self.model)
        if self.rank == 0:
    ...
    def save_ckpt(self, ckpt_name, update_best_ckpt=False):
        if self.rank == 0:
            ...
            logger.info("Save weights to /pretrained")
            ckpt_state = {
