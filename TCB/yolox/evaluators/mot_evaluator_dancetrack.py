...
from yolox.tracker.my_byte_tracker_dance import BYTETracker###############################################################
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                if video_name not in video_names:
                    video_names[video_id] = video_name
                if frame_id ==1:
                    tracker = BYTETracker(self.args,params)#new tracker
                    if len(results)!=0:#write last video
                        result_filename = os.path.join(result_folder, '{}.txt'.format(video_names[video_id - 1]))
                        write_results(result_filename, results)
                        results = []
                imgs = imgs.type(tensor_type)
                ...
                outputs = model(imgs)#[batchsize,h1*w1+h2*w2+h3*w3,262]
                outputs = postprocess(outputs.cpu(),self.num_classes,self.confthre,self.nmsthre)
                #get nms-selected bboxes
                
                if is_time_record:
                    infer_end = time_synchronized()
                    inference_time += infer_end - start
            output_results = self.convert_to_coco_format(outputs,info_imgs,ids)
            #note:scale到原图的bbox，而不只是xyxy->xywh
